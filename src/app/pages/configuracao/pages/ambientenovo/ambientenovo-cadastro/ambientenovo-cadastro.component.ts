...
import { Idioma } from '@radoccmodels/base/idioma';
import { IdiomaService } from '@radoccservices/base/idioma-service';
...
import { Plano } from '@radoccmodels/base/plano';
...
import { PeriodoService } from '@radoccservices/base/periodo-service';
import { PlanoService } from '@radoccservices/base/plano-service';
import { AdicionalService } from '@radoccservices/base/adicional-service';
...
export class AmbienteNovoCadastroComponent extends CadForm implements OnInit {
    ...
    public findPlanoPorNome(nome: string) {
        this.planoService.buscarPorNome(nome).subscribe((lista: Plano[]) => {
            this.listaPlano = lista;
        });
    }
    public findPeriodoPorNome(nome: string) {
        this.periodoService.buscarPorDescricao(nome).subscribe((lista: Periodo[]) => {
            this.listaPeriodo = lista;
        });
    }
    public findIdioma(nome: string) {
    ...
    public salvar($event) {
        if (this.form.invalid) {
            this.page.showErrorMsg('EXISTEM_CAMPOS_INVALIDOS');
            return;
        }
    ...
    public buscarDados() {
        ...
        this.planoService.buscarPlanosComercial().subscribe(data => {
            ...
            this.page.showErrorMsg('OCORREU_UM_ERRO_AO_BUSCAR_DADOS');
        })
    ...
    public buscarAdicional(sair?) {
        ...
        this.adicionalService.filtrarPorLicenca(this.licenca.id).subscribe((data: FuncionalidadeAdicional[]) => {
            ...
            // this.loading = false;
            this.page.showErrorMsg('ERRO_AO_BUSCAR');
        })
    }
